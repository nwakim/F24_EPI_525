...
1.  Set the [**level of significance**]{style="color:#3070BF"} $\alpha$

2.  Specify the [**null**]{style="color:#3070BF"} ( $H_0$ ) and [**alternative**]{style="color:#3070BF"} ( $H_A$ ) [**hypotheses**]{style="color:#3070BF"}

    1.  In symbols
    2.  In words
    3.  Alternative: one- or two-sided?

3.  Calculate the [**test statistic**]{style="color:#3070BF"}.

4.  Calculate the [**p-value**]{style="color:#3070BF"} based on the observed test statistic and its sampling distribution

5.  Write a [**conclusion**]{style="color:#3070BF"} to the hypothesis test
...
- When testing a statistical hypothesis, we specify a significance level, $\alpha$
  - Typically choose $\alpha = 0.05$
- An $\alpha$ level of 0.05 implies that an event occurring with probability lower than 5% will be considered sufficient evidence against the null hypothesis
- This is parallel to our confidence interval
  ...
  - A ($1-\alpha$)% confidence interval will have about $\alpha$% confidence intervals that do not cover $\mu$ even though they come from the distribution with mean $\mu$
...
-   [Typical $\alpha$]{style="color:darkblue"} values are
    ...
    -   0.01 and 0.1
-   Decision rule:
    -   When [$p$-value \< $\alpha$]{style="color:green"}, we "[**reject**]{style="color:green"} the null hypothesis [$H_0$]{style="color:green"}."
    -   When [$p$-value $\geq \alpha$]{style="color:purple"}, we "[**fail to reject**]{style="color:purple"} the null hypothesis [$H_0$]{style="color:purple"}."
...
-   Replace hand calculation in step 4 and 5 with R!!
-   Run a t-test in R
-   `tidy()` the test output using `broom` package
...
- We calculate $P(Z \geq |{z_{\overline{X}}}|)$ = `pnorm($z_{\overline{X}}$)`
- OR: We calculate $P(X \geq |\overline{x}|)$ = `pnorm($\overline{x}$, $\mu$, $SE$)`
