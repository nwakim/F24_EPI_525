...
1.  Understand how CLT applies to a binomial random variable

2.  Construct and interpret confidence intervals for a single proportion.

3.  Run a hypothesis test for a single proportion and interpret the results.
...
-   The methods used can be extended to **categorical data**, such as binomial proportions or data in two-way tables

-   Categorical data arise frequently in medical research because disease outcomes and patient characteristics are often recorded in natural categories such as types of treatment received, whether or not disease advanced to a later stage, or whether or not a patient responded initially to a treatment
...
-   Standard deviation: $\sqrt{n \cdot \hat{p} \cdot (1-\hat{p})}$
-   Variance: $n \cdot \hat{p} \cdot (1-\hat{p})$
:::
:::::
...
-   In a study where 52 patients were treated concurrently with two new therapies, nivolumab and ipilimumab, 21 had an immune response.[^1]

-   Our outcome that we are measuring is whether or not each person has an immune response
...
```{r}
prop.test(x = 21, n = 52, conf.level = 0.95)
```
...
## 95% CI for population proportion (using R)
...
```{r}
#analyze the data
prop.test(x = c(500, 505), n = c(500 + 44425, 505 + 44405), correct=T)
```
